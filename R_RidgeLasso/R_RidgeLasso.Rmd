...
Once you have selected a model, for instance the model with  $\lambda_{1se}$, you can also use the `predict` function to use the estimated model to produce model forecasts. Typically you will have a new set of observations. Here we will produce forecasts for the observations which we did use for estimation. To produce forecasts you will need a matrix `x` which has the correct dimension. Recall what the dimension of the matrix `x` is
...
You can feed in any number of observations but you will need to ensure that the new input matrix has the same column structure (number of columns and same ordering of variables) as the `x` used in estimating the model. As we will ise `x` itself, this is trivially the case.

```{r}
ridge.predict.lambda.1se=predict(cvridge, x, type="response",s=cvridge$lambda.1se)
```

If you have a different set of observations you feed the respective matrix into `predict` instead of `x`. Here the result is a vector of `r dim(x)[1]` predictions which we can compare to the original observations of hourly wages which are stored in `y`.

```{r}
yp_ridge <- as.vector(ridge.predict.lambda.1se) # as vector makes a vector 
temppred <- data.frame(y, yp_ridge )
ggplot(temppred, aes(y = yp_ridge, x = y)) + 
  geom_point() +
...
The plot only shows the variables with coefficients that have been estimated to be unequal to 0. These are only 30 out of 136.

As for the Ridge regression you will eventually have to chose a $\lambda$ value.
