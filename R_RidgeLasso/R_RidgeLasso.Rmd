...
PSID <- PSID %>% mutate(across(cols,factor))
```

`r hide("How does this line work?")`

Understanding lines of code is a very important skill. In this code you will understand that there is some piping going on and that somehow the dataframe `PSID` is manipulated, but especially the part `mutate(across(cols,factor))` May appear a little murky.

AI is really good at explaining code in plain English. You can copy the line of code and paste it into your favourite LLM and ask it to explain what this line of code in R does and you will get a good explanation.
...
What is the meaning of the `family = "gaussian"` option in the call to `glmnet`?
...
When you compare the plots make sure you note the different scales on the horizontal axis. Larger $\lambda$s result in smaller coefficient values. The vertical axis shows the associated variables, but there are too many to read these properly.
...
Once you have chosen a $\lambda$ you can get the coefficients and predictions from that particular model. Here we look at the coefficients from the optimal model using both $\lambda_{min}$ and $\lambda_{1se}$. To do that we use the `predict` function using the `type="coeff"` option.
