...
These are clearly more symmetric than `RV` itself. In a later section we will apply HAR  type models to such transformations.
...
As you have seen above, assembling the new information required to apply the `predict` function, in order to make out of sample forecasts is a little messy. And in fact we used information which we previously hand calculated, the variables `RV1`, `RV5` and `RV22`. But at the same time we saw above that the `HARmodel` function does internally calculate these values. Let's see whether we can use the `HARmodel` function to do some of that dirty work for us. Recall that the aim was to get $E[RV_{4096}^5|I_{4091}]$. For that reason for first calculated coefficients using information $I_{4091}$. They are stored in `mod_HAR5`. In the creation of that we used `data_temp <- rv_data[index(rv_data)<="2013-08-23","RV"]` or `data_temp <- rv_data[1:4091,"RV"]` acknowledging that "2013-08-23" is day 4091.
...
```{r}
est_info$beta0_1 <- NA   # four HAR coefficients ot be saved
est_info$beta1_1 <- NA
est_info$beta2_1 <- NA
est_info$beta3_1 <- NA
est_info$f1 <- NA
...
for(j in 1:est_samples) {
  ...
  # as demonstrated above we, need the explanatory variable from en+1
  est_info$f1[j] <- predict(mod_HAR_loop,newdata = rv_data[en+1,c("RV1","RV5","RV22")])
  # and we need the realisation from en+h
  est_info$r1[j] <- rv_data[en+1,"RV"]
  
  # Estimate the HAR for logRV
...
In particular we could think of estimating standard HAR models on transformations of $RV_t$, of extensions to the standard HAR model and combinations thereof.
