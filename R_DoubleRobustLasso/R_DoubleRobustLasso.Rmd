...
The two variables we are particularly interested here is the variable labelled `Outcome` which here is defined as a GDP growth rate over a decade (from 1965 to 1975) for a particular country. The covariate we are particularly interested in is `gdpsh465` which represents the initial (i.e. in 1965) level of $log(GDP per capita)$. The question here is whether countries which are poorer at the beginning of that growth period grow faster, ceteris paribus, implying a negative relation between `Outcome` and `gdpsh465` after controlling for relevant covariates. 

The question then is what relevant covariates are?
...
If you wanted to identify countries you could go to the data exercise linked above and attempt to match variable outcomes, say for `seccf65` to identify countries.
...
As you can see the majority of countries display positive growth rates. over a 10 year period.
...
* ration of gvernment consumption (not incl defence and education) to GDP, `gvxdxe41`
