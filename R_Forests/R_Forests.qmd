...
It is important to understand that trees are really prediction tools. It is not a tool, by itself, to discover causal relationships.
...
A single tree tends to overfit the data it's fed, resulting in incorrect predictions when tested on new data. This is a very common issue for any nonlinear models (e.g. neural networks) and when you use these methods you need to be aware of this and guard against this effect. 

One strategy to counter this is to evaluate the predictive performance by splitting the data into an estimation and a test data set. The model is estimated using the estimation data but the predictive performance is evaluated using the data in the test set. In this way, fitting particular features that are unique to the data in the estimation set but do not generalise is not rewarded. Bagging and random forests are additional methods to deal with this particular problem. They will be discussed towards the end of this walkthrough. 
...
We use the 'banks.csv' file, which has 10% of the observations in the original data (randomly chosen) and fewer input variables. Using this smaller file makes it easier to run machine‑learning methods that require more computing power. We recommend that you engage with this walkthrough using the reduced dataset in 'banks.csv'. The full dataset is also available from the downloaded zip file.
...
The data consists of 17 variables and 4521 observations. The outcome of interest is the one simply called `y`. `y = no` indicates the client did not subscribe to the term deposit, `y = yes` indicates that a client did.
...
Only slightly more than a tenth of clients subscribed to term deposits during the telemarketing campaign. Such imbalance is quite common in real-world data. However, this results in machine-learning models becoming very good at predicting the majority class (`y = no`), while struggling to learn patterns related to the minority class (`y = yes`). There are specialised techniques for addressing this issue, but to keep the exercise straightforward, we will not apply them here. The final discussion will return to this point.

::: {.callout-important}
### Naive prediction

From the above table for `y` you can see that a naive prediction algorithm would work quite well. We could just always predict "no". In that case we would get about 89% of all predictions right. This is what we will later call "accuracy". So we will want to improve on that. This is useful to keep in mind.

Further, this naive method would never predict a "yes". Perhaps it is your reason to undertake this exercise to predict the rare occasions of "yes". In that case the naive method is really not useful.
...
To fit a tree to the data and evaluate it for its accuracy in prediction, we place the observations randomly into training and test sets. The estimation/training set will be used to fit a classification tree that models `y`. The training set is then used to evaluate the quality of the model. 

Variables other than `y` will be provided as inputs (or features or explanatory variables).
...
You may also know that computers do not use real random numbers. Setting the random seed ensures that you do draw the same random numbers which means that when you run the code repeatedly you always get the same "random" selection and subsequent results remain unchanged.
...
### Estimating a toy example

In order to explain the intuition of a tree we will begin by estimating a small tree which only uses two explanatory variables, here `duration` and `month`.


```{r}
# fits a tree to the training data
tree.bank.toy <- tree(y ~ duration+month, train_data)                                        
```
...
knitr::include_graphics("../images/tree_toyA.png")
```

In order to explain what the meaning of this tree is we labelled the leafs or final partitions. We can call these partitions as we can understand these as partitioning the space of the explanatory variables. As we restricted this toy example to two variables we can visualise this in the following partitioning where the latter correspond to the previous image. (Also note that the `month` variable is a factor variable without a natural ordering. In the picture the months have been ordered in a way that faciliatates the graphical representation.)

```{r echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("../images/tree_toyB.png")
```

The tree method divisions up the entire space into areas. In each area we also see the prediction of "yes" or "no", with only areas C and G having a majority of predictions being "yes". This is not surprising, remembering that only about 11% of observations are "yes".

### Estimating the full tree

By restricting the toy example to two explanatory variables we could still represent the space of the explanatory variables graphically. When we generalise to more explanatory variables the same principle applies, but any attempts to represent the higher dimensional space graphically will not work.

Now we can estimate (`fit`) the full tree using the training data (`train_data`).
...
The tree algorithm aims to maximise node purity, and so the the classification probabilities will tend towards 0 or 1. Note, there are 13 terminal nodes and 12 internal nodes giving rise to 25 outcome probabilities.
