...
In order to explain what the meaning of this tree is we labelled the leafs or final partitions. We can call these partitions as we can understand these as partitioning the space of the explanatory variables. As we restricted this toy example to two variables we can visualise this in the following partitioning where the latters correspond to the previous image. (Also note that the `month` variable is a factor variable without a natural ordering. In the picture the months have been ordered in a way that faciliatates the graphical representation.)

```{r echo=FALSE, fig.align = "center"}
knitr::include_graphics("../images/tree_toyB.png")
```

The tree method divisions up the entire space into areas. In each area we also see the predicyion of "yes" or "no", with only areas C and G having a majority of predictions being "yes". This is not surprising, remembering that only about 11% of observations are "yes".
